  ...
  return (
    <nav className='navbar navbar-expand-lg p-4' style={{backgroundColor: '#445D48'}}>
      <div className='container-fluid'>
        <Link to='/' className="navbar-brand nav-link fw-bold" style={{color: '#FDE5D4'}}>ELEVATE</Link>
      ...
      <div className="collapse navbar-collapse" id="navbarNavDropdown">
        <ul className="navbar-nav ms-auto d-flex flex-row">
          <li className='nav-item dropdown'>
            ...
            {services.length > 0 && (
            <ul className='dropdown-menu' style={{backgroundColor: '#F4F1DE'}}>
            {services.map((oneService)=>(
              <li key={oneService.id}><a style={{color: '#001524'}} className="dropdown-item" href={`/services/${oneService.id}`}>{oneService.name}</a></li>
            ))}
          </ul>
          )}
